...
data_provider = DatasetProvider(
    ...
    # what type of data provider is this?  Must be a pre-defined value from Orchestra's library, although a user can choose to implement their own DatasetProvider
    type=DatasetProvider.types.XXX,
    # what Dataset is this provider for?
    data_source=Dataset(),
    ...
    # per DatasetProvider.type configuration, for example
...
# TODO: Is there a better name for Dataset?  I think right now it might be a bit confusing, it could be something like "DataSchema" but that feels like it might be a sub-property of a DatasetProvider rather than the intended top-level entity...
...
# TODO: Do we need a way to run DataCode() on a DatasetProvider?  or, should this be handled by each provider's config?  An example use case might be changing column names, dropping rows that are >x% null, filtering out known bad rows, etc.  I lean to yes...
...
def python_data_code(row_values: dict, data_lookups: dict) -> dict:
    ...
    # data_lookups contains the joined rows input_Datasets
